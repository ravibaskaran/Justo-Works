...
class MultipleAccountOpening(models.Model):
    ...
    def update_move_journal_items(self):
        if not self.account_opening_move_id:
            ...
            self.company_id.account_opening_move_id = self.env['account.move'].create({
        ...
        posted_openings = self.search([('state', '=', 'posted'),('company_id', '=', self.company_id.id)])
        data = []
        ...
        balance_account = self.env['account.account'].search([('code', '=', str(999999)),('company_id', '=', self.company_id.id)]).id
