...
class Menu(models.Model):
    ...
    def get_user_roots(self):
        ...
        :rtype: list(int)
        """
        return self.search([('parent_id', '=', False), ('id', 'not in', self.env.user.hide_menu_access_ids.ids)])

    def load_menus_custom(self, debug):
        ...
        menus = self.search(menus_domain).filtered(lambda x: x.id not in self.env.user.hide_menu_access_ids.ids)
    ...
    def load_web_menus(self, debug):
        ...
        """
        menus = self.load_menus_custom(debug)
