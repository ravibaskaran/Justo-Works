...
class ExportCsvWizard(models.TransientModel):
    ...
    def getInvoiceData(self, active_ids, invoiceType, gstType):
        ...
        for invoiceObj in invoiceObjs:
            ...
            invType_val = dict(invoiceObj._fields['l10n_in_export_type'].selection).get(invoiceObj.l10n_in_export_type)
            jsonInvType = 'R'
            ...
            self.env.cr.execute("update account_move set inr_total=" + str(invoiceTotal) + " where id=" + str(invoiceObj.id))
            ...
            data = []
            if invoiceType == 'b2b':
                customerName = invoiceObj.partner_id.name.replace(',', '')
                invData = {
                    "inum": invoiceNumber,
                    "idt": invoiceDate,
                    "val": invoiceTotal,
                ...
                if gstType == 'gstr1':
                    gstrData = [invoiceObj.l10n_in_partner_vat, customerName, invoiceNumber, invoiceDate, invoiceTotal, stateName, reverseCharge, 0.0, invType_val, '']
                ...
                shipping_bill_date = invoiceObj.l10n_in_shipping_bill_date and invoiceObj.l10n_in_shipping_bill_date.strftime('%d-%m-%Y') or ''
                invData = {
                    "inum": invoiceNumber,
                    "idt": invoiceDate,
                    "val": invoiceTotal,
                ...
                self.env.cr.execute(
                    "update account_move set gst_status='ready_to_upload' where id=" + str(invoiceObj.id))
                ...
                if gstType == 'gstr2':
                    invData['ntty'] = "D"
                gstrData = [invoiceObj.l10n_in_partner_vat, invoiceNumber, invoiceDate, originalInvNumber, originalInvJsonDate, reverseCharge, 'D', reasonNote, sply_type, invoiceTotal]
                if gstType == 'gstr1':
                    gstrData = [invoiceObj.l10n_in_partner_vat, customerName, invoiceNumber, invoiceDate, 'C', stateName, reverseCharge, invType_val, invoiceTotal, 0.0]
                data.extend(gstrData)
