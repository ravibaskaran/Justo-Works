...
class RentalContract(models.Model):
    ...
    def _default_security_deposit_account(self):
        security_deposit_account = self.env['ir.config_parameter'].sudo().get_param(
            'itsys_real_estate.security_deposit_account')
    ...
    def auto_rental_invoice(self):
        try:
            ...
            rental_line_ids = rental_pool.search(
                [('loan_id.state', '=', 'confirmed'), ('date', '<=', fields.Date.today())])
    ...
    region = fields.Many2one('regions', 'Cluster', )
    ...
    account_analytic_id = fields.Many2one('account.analytic.account', string='Analytic Account',
                                          related='building_unit.rel_anyletec_prop')
    # ,
    ...
    property_owner_id = fields.Many2one('res.partner', string="Developer", domain=[('is_owner', '=', True)])
    ...
    def unlink(self):
        ...
        super(RentalContract, self).unlink()
    ...
    def create(self, vals):
        ...
        new_id = super(RentalContract, self).create(vals)
        return new_id
        ...
        for rec in self:
            if rec.periodicity and rec.date_from and rec.date_to:
                ...
                if self.insurance_fee:
                    rental_lines.append(
                        (0, 0, {'serial': i, 'amount': self.insurance_fee, 'date': date_from,
                                'name': _('Insurance Deposit')}))
                    i += 1
                rental_lines.append(
                    (0, 0, {'serial': i, 'amount': rental_fee, 'date': date_from, 'name': _('Rental Fee')}))
                i += 1
                ...
                while new_date < (date_to - relativedelta(**{periodicity: self.recurring_interval})):
                    ...
                    rental_lines.append(
                        (0, 0, {'serial': i, 'amount': rental_fee, 'date': new_date, 'name': _('Rental Fee')}))
                    i += 1
                self.write({'loan_line': rental_lines})


class LoanLineRsRent(models.Model):
