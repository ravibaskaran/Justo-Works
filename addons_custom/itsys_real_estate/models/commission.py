                    ...
                    if commission.apply_to == 'salesman':
                        ...
                        if user_exist:
                            if commission.commission_type == 'fixed':
                                if commission.contract_state == 'open':
                                    total_commission += record.amount_total * commission.fix_qty / 100.0
                            else:  # by section
                                if commission.contract_state == 'open':
                                    total_commission += commission.calculate_section(record.amount_total)
                        ...
                        if user_exist:
                            if leader:
                                if commission.commission_type == 'fixed':
                                    if commission.contract_state == 'open':
                                        total_commission += record.amount_total * commission.leader_fix_qty / 100.0
                                else:  # by section
                                    if commission.contract_state == 'open':
                                        total_commission += record.amount_total * commission.leader_fix_qty / 100.0
                            else:
                            ...
                            if leader:
                                ...
                                if commission.commission_type == 'fixed':
                                    ...
                                    if commission.contract_state == 'paid':
                                        total_commission += installment_amount * commission.fix_qty / 100.0
                    if total_commission > 0:
                        comm_line_obj.create({'date': commission_date, 'salesman': user, 'amount': total_commission,
                                              'commission': record.id})
