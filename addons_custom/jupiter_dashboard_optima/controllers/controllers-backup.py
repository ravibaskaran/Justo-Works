...
class JupiterDashboardOptima(http.Controller):
    ...
    def bookings_registrations_region_wise2(self, frequency='today', region_ids=False, count_or_value='count',
                                            region_or_cluster='region', parent_region=False, project_ids=False,
                                           custom_start=False, custom_end=False,month_or_quarter='month', financial_year=False,show_booking=True, show_cancel=True, show_net=True, show_reg=True):
        ...
        if month_or_quarter == 'month':
            ...
            # for i in range(12):
                ...
                last_day_of_month = first_day_of_month + relativedelta(day=31)  # last day of the month

                # Set start and end of the month (midnight to 23:59)
                start_of_month = first_day_of_month.replace(hour=0, minute=0, second=0)
                ...
                if show_booking:
                    ...
                    table_dict['Booking']['datas']['data'].append(booking_confirmed_value)
                    table_dict['Booking']['datas']['current_month'].append(current_month)
                    table_dict['Booking']['total'] += booking_confirmed_value
                else:
                    series[0]['data'].append(0)  # Bookings (Confirmed)
                    table_dict['Booking']['datas']['data'].append(0)
                    table_dict['Booking']['total'] += 0
                if show_cancel:
                    series[1]['data'].append(booking_cancelled_value)  # Cancellations
                    table_dict['Cancellation']['datas']['data'].append(booking_cancelled_value)
                    table_dict['Cancellation']['datas']['current_month'].append(current_month)
                    table_dict['Cancellation']['total'] += booking_cancelled_value
                else:
                    series[1]['data'].append(0)  # Cancellations
                    table_dict['Cancellation']['datas']['data'].append(0)
                    table_dict['Cancellation']['total'] += 0
                if show_net:
                    series[2]['data'].append(net_value)  # Net
                    table_dict['Net']['datas']['data'].append(net_value)
                    table_dict['Net']['datas']['current_month'].append(current_month)
                    table_dict['Net']['total'] += net_value
                else:
                    series[2]['data'].append(0)  # Net
                    table_dict['Net']['datas']['data'].append(0)
                    table_dict['Net']['total'] += 0
                if show_reg:
                    series[3]['data'].append(registration_value)  # Registrations
                    table_dict['Registration']['datas']['data'].append(registration_value)
                    table_dict['Registration']['datas']['current_month'].append(current_month)
                    table_dict['Registration']['total'] += registration_value
                else:
                    series[3]['data'].append(0)  # Registrations
                    table_dict['Registration']['datas']['data'].append(0)
            ...
            # for i in range(12):
                ...
                # Correctly create the datetime object for the last day of the quarter
                quarter_end_date = datetime(first_day_of_month.year, quarter_end_month, 1) + relativedelta(day=31)
        ...
        for tb in table_dict:
            ...
            t_body += '<tr>'
            t_body += '<td>'+tb+'</td>'
            for i, tb2 in enumerate(table_dict[tb]['datas']):
                t_body += f'<td class="data-table-{tb}-td data_column" data-type="{tb}" data-month="{current_date.strftime("%B %Y")}" data-value="{tb2['data'][i]:.2f}">{tb2['data'][i]:.2f}</td>'
            t_body += '<td class="data-table-'+tb+'-td tot_td">'+f'{table_dict[tb]["total"]:.2f}'+'</td>'
            t_body += '</tr>'
