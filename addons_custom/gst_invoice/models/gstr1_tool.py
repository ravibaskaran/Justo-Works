...
class Gstr1Tool(models.Model):
    ...
    outward_attachment = fields.Many2one('ir.attachment',help="Outward supply attchment")
    ...
    def reset(self):
        ...
        if self.outward_attachment:
            self.outward_attachment.unlink()
        ...
        self.message_post(body=_(body), subtype_id=subtype_id.id)
        return True
    ...
    def updateInvoiceCurrencyRate(self, invoiceObjs):
        for invoiceObj in invoiceObjs:
            ...
            self.env.cr.execute(
                "update account_move set inr_total='" + str(amount_total) + "' where id=" + str(invoiceObj.id))
        return True
                ...
                if invoiceObj.partner_id.country_id.code == 'IN':
                    if invoiceObj.partner_id.vat:
                        ...
                        if not invoiceObj.bonded_wh:
                            ...
                            self.env.cr.execute(
                                "update account_move set bonded_wh='no' where id=" + str(invoiceObj.id))
                    else:
                    ...
                    invoice_type = 'export'
                    self.env.cr.execute(
                        "update account_move set export='WOPAY' where id=" + str(invoiceObj.id))
            if invoice_type:
                self.env.cr.execute("update account_move set invoice_type='" + invoice_type + "' where id=" + str(invoiceObj.id))
        return True
    ...
    def generateCsv(self):
        ...
        if invoiceObjs:
            name = self.name
            if self.period_id and self.period_id.name:
                name += "_" + self.period_id.name
            typeDict = {}
            ...
            branches = self.env['account.journal'].search([('type', '=', 'sale')])
            branch_inv = []
            binv_val = []
            n = 0
            for branch in branches:
                inv_list = []
                for line in invoiceObjs.filtered(lambda l: l.move_type == 'out_invoice'):
                    if line.journal_id.id == branch.id:
                        inv_list.append(line.name)
                if len(inv_list) > 0:
                    n += 1
                    branch_inv.append({
                        'num': int(n),
                        'from': inv_list[-1],
                        'to': inv_list[0],
                        'totnum': int(len(inv_list)),
                        'cancel': 0,
                        'net_issue': int(len(inv_list))
                    })
                    binv_val.append([
            ...
            for branch in branches:
                inv_list = []
                for line in invoiceObjs.filtered(lambda l: l.move_type == 'out_refund'):
                    if line.journal_id.id == branch.id:
                        inv_list.append(line.name)
                if len(inv_list) > 0:
                    n += 1
            ...
            #     i += 1
            # print(final_list)
            jsonData.update({'doc_issue': {"doc_det": [{"0": {"doc_num": 1,
                                                              "doc_type": "Invoices for outward supply",
                                                              "docs": branch_inv,
                                                              }}]}})
            fp = io.StringIO()
            writer = csv.writer(fp, quoting=csv.QUOTE_NONE, escapechar='\\')
            inv_head = ['Document Type', 'Number', 'From', 'To', 'Total Line Number', 'Cancel', 'Net Issue']
            writer.writerow([_unescape(name) for name in inv_head])
            for lineData in binv_val:
                writer.writerow([_unescape(name) for name in lineData])
            fp.seek(0)
            data = fp.read()
            fp.close()
            attachment = self.env['export.csv.wizard'].generateAttachment(data, 'outward', 'supply')
            if attachment:
                self.outward_attachment = attachment.id
            ...
            #         if line.branch_id.id == branch.id:
            #             inv_list.append(line.name)
            #     if len(inv_list) > 0:
            #         n += 1
            #         branch_inv.append({
    ...
    def exportOutwardCSV(self):
        if not self.outward_attachment:
            self.generateCsv()
        if not self.outward_attachment:
            raise UserError("Outward attachment is not present")
        return {
            'type': 'ir.actions.act_url',
            'url': '/web/content/%s?download=1' % (self.outward_attachment.id),
            'target': 'new',
        }
    ...
    def updateInvoiceStatus(self, status):
        for lines in self.invoice_lines:
            self.env.cr.execute(
                "update account_move set gst_status='" + str(status) + "' where id=" + str(lines.id))
            # lines.write({'gst_status': status})
        return True


    # def branch_hsn_report(self):
    #     return {
    #         'name': ("Branch HSN Report"),
    #         'view_mode': 'form',
    #         'res_model': 'branch.hsn.report',
    #         'view_id': self.env.ref('gst_invoice.branch_hsn_report_wizard').id,
    #         'type': 'ir.actions.act_window',
    #         'nodestroy': True,
    #         'target': 'new',
    #     }