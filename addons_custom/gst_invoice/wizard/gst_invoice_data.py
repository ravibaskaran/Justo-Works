            ...
            if invoiceLineData:
                ...
                if gstType == 'gstr1':
                    if rate not in rateDict.keys():
                        ...
                        rateDataDict[rate]['taxval'] = rateDataDict[rate]['taxval'] + rateAmount
                        rateDataDict[rate]['cess'] = rateDataDict[rate]['cess'] + cessAmt
                if rate not in rateJsonDict.keys():
                    # print(invoiceLineData)
                    ...
                    for key in invoiceLineData[1].keys():
                        ...
                        if rateJsonDict[rate].get(key):
                            ...
                            # print(rateJsonDict[rate][key], invoiceLineData[1][key],'kfuij')
                invData = []
                if gstType == 'gstr1':
    ...
    def getInvoiceLineData(self, invoiceLineData, invoiceLineObj, invoiceObj, invoiceType):
        ...
        if rateObjs:
            ...
            for rateObj in rateObjs:
                ...
                taxedAmount = taxData[0]
                taxJsonLineData = self.env['gst.tax.data'].getGstTaxData(
                    invoiceObj, invoiceLineObj, rateObjs, taxedAmount, invoiceType)
                if rateObj.amount_type == "group":
                    for childObj in rateObj.children_tax_ids:
                        rate += childObj.amount * 2
                        lineData.append(rate)
                        jsonLineData['camt'] = taxJsonLineData.get('camt')
                        jsonLineData['samt'] = taxJsonLineData.get('samt')
                        break
                else:

                    if 'cess' in rateObj.name.lower():
                        cess += round(taxData[0], 2)
                        jsonLineData['csamt'] = taxJsonLineData.get('csamt')
                    else:
                        rate += rateObj.amount
                        lineData.append(rate)
                        jsonLineData['iamt'] = taxJsonLineData.get('iamt')
                # break
        ...
        return [data, jsonLineData, rate, rateAmount, cess]
