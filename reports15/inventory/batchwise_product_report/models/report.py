...
class ReportBatchwiseProduct(models.TransientModel):
    _name = 'batchwise.product'
    _inherit = 'beta.reports'
    _description = 'Batchwise Product report'
    ...
    def get_report_values(self, data=None):
        ...
        return {
            ...
            'products': products,
            'location': location,
            'lines': self.get_batchwise_product(data),
        }

    def get_batchwise_product(self, data):
        ...
        if data['date_from'] and data['date_to'] and data['location_ids']:
            ...
            movelines = move_line_obj.search(domain, order="lot_id,date asc")
            # for1st = time.time()
                ...
                if partner == '':
                    ...
                    if 'is_transfer_receipt' in line.picking_id._fields:
                        if line.picking_id.is_transfer_receipt == True:
                            branch = self.env['res.branch'].search(
                                [('destination_warehouse_id.lot_stock_id', '=', line.location_id.id)],
                                limit=1).name
                            ...
                            if [line.location_dest_id.id] != data['location_ids']:
                                ...
                                branch = self.env['res.branch'].search(
                                    [('destination_warehouse_id.lot_stock_id', '=', line.location_dest_id.id)],
                                    limit=1).name
                                if branch:
                                    partner += ' to ' + branch
                    ...
                    if 'type' in line.picking_id._fields:
                        if line.picking_id.type == True:
                            ...
                            if [line.location_dest_id.id] == data['location_ids']:
                                branch = self.env['res.branch'].search(
                                    [('destination_warehouse_id.lot_stock_id', '=', line.location_id.id)],limit=1).name
                                partner = 'Transfer Receipt'
                                if branch:
                                    partner += ' from ' + branch
                ...
                if data['horizontal_view']:
                    if line.product_id.id in datas:
                        ...
                        opening_data = self.get_product_opening(line.product_id, data['date_from'],
                                                                data.get('allowed_company_ids'), data['location_ids'], False)
                    ...
                    # if line.lot_id:
                    if line.product_id.id in datas:
                        if line.lot_id.id in datas[line.product_id.id]['data']:
                            datas[line.product_id.id]['data'][line.lot_id.id]['lots'].append((move_date, partner, name, type, in_qty, out_qty))
                        else:
                            lot_opening = self.get_product_opening(line.product_id, data['date_from'],
                                                                   data.get('allowed_company_ids'),
                                                                   data['location_ids'], line.lot_id)
                            datas[line.product_id.id]['data'][line.lot_id.id] = {'lots': [(move_date, partner, name, type, in_qty, out_qty)],
                                                                              'opening_issue': lot_opening[1],
                                                                              'opening_receipt': lot_opening[0],
                                                                              'opening_balance': lot_opening[0] - lot_opening[1],
                                                                                 'lot': line.lot_id,
                                                                              }
                        ...
                        opening_data = self.get_product_opening(line.product_id, data['date_from'],
                                                                data.get('allowed_company_ids'), data['location_ids'], False)
                        lot_opening = self.get_product_opening(line.product_id, data['date_from'],
                                                                data.get('allowed_company_ids'), data['location_ids'], line.lot_id)
                        datas[line.product_id.id] = {
                            'name': line.product_id.name,
                            'data': {line.lot_id.id: {'lots': [(move_date, partner, name, type, in_qty, out_qty)],
                                                   'opening_issue': lot_opening[1],
                                                   'opening_receipt': lot_opening[0],
                                                   'opening_balance': lot_opening[0] - lot_opening[1],
                                                      'lot':line.lot_id,
    ...
    def get_product_opening(self, product, date, companies, location, lot):
        actual_date = self.get_actual_date(date + " 00:00:00")

        move_obj = self.env['stock.move.line']
        in_domain = [('product_id', '=', product.id), ('date', '<', actual_date), ('state', '=', 'done')]
        out_domain = [('product_id', '=', product.id), ('date', '<', actual_date), ('state', '=', 'done')]
        if location:
            in_domain.append(('location_dest_id', 'in', location))
            out_domain.append(('location_id', 'in', location))
        else:
            locations = self.env['stock.location'].search([('company_id', 'in', companies), ('usage', '=', 'internal')])
            in_domain.append(('location_dest_id', 'in', locations.ids))
            out_domain.append(('location_id', 'in', locations.ids))
        if lot:
            in_domain.append(('lot_id', '=', lot.id))
            out_domain.append(('lot_id', '=', lot.id))
        # if location:
            # locations = self.env['stock.location'].search([('company_id','in',companies),('usage','=','internal')])
    ...
    def get_html(self):
        res = self._get_report_data(date_from=self.date_from.strftime('%Y-%m-%d'), date_to=self.date_to.strftime('%Y-%m-%d'),
                                    product=self.product,location=self.location,horizontal_view=self.horizontal_view)
        self.template_area = self.env.ref('batchwise_product_report.report_batchwise_product')._render(
        ...
        # print("111",res['lines']['lines'])
        return res


    def _get_report_data(self, date_from=False, date_to=False, product=False, location=False,horizontal_view=False):
        ...
        if 'branch_id' in self.env.user._fields:
            ...
            if location:
                 branch_name = self.env['stock.warehouse'].search([('lot_stock_id','=',location)]).branch_id.name
        ...
        data = {
            ...
            'horizontal_view': horizontal_view if horizontal_view else False,
