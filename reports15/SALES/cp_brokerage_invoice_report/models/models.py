...
class BetaCpBrokerageInvoiceReport(models.TransientModel):  # change this
    _name = 'beta.cp.brokerage.invoice.report'  # change this
    _inherit = 'beta.reports'
    _description = "CP Brokerage Invoice Report"
    ...
    def default_get(self, fields_list):
        res = super(BetaCpBrokerageInvoiceReport, self).default_get(fields_list)
...
account_tax AS tax ON tax.id = aml.tax_line_id
                ...
                am.move_type = '""" + str(type1) + """'  and am.state='posted' and rp.is_channel = True and am.project_invoice_type = 'cp_brokerage'
                """ + domain + """ GROUP BY 
        ...
        if customer == 'all' and detailed == False:
            ...
            self.env.cr.execute(
                """select rp.name, COUNT(partner_id) as count , SUM(am.amount_total) as total, SUM(am.amount_residual) as due from account_move as am left join res_partner as rp ON rp.id = am.partner_id where 
                                    ...
                                    and am.state='posted' and rp.is_channel = True
                                   and am.project_invoice_type = 'cp_brokerage' """ + domain + """ group by rp.name order by rp.name""")
            for dt in self.env.cr.dictfetchall():
...
where aml.tax_line_id IS NOT NULL and 
                    ...
                    customers) + """'  and am.state='posted' and am.project_invoice_type = 'cp_brokerage' 
                                                    and rp.is_channel = True """ + domain + """ GROUP BY 
am.id, rp.id order by rp.name """)
                for dt in self.env.cr.dictfetchall():
                    docs.append({
                        'id': dt['id'],
    ...
    def get_html(self):
        ...
        self.template_area = self.env.ref('cp_brokerage_invoice_report.report_cp_brokerage_invoice')._render(
