            ...
            if (!self.hierarchy_groups) {
                prom = this._rpc({
                    ...
                    self.hierarchy_groups = [
                        {'name': _t('Trading Accounts'), 'ids': _.map(_.filter(arg, v => v['internal_group'] == 'trading_account'), v => v['id'])},
                        {
                            'name': _t('Profit & Loss'),
                            'children': [
                        ...
                        },
                        {
                            ...
                            'ids': _.map(_.filter(arg, v => v['internal_group'] == 'bs_other'), v => v['id']),
                            'children': [
                        ...
                        {'name': _t('Manufacturing Accounts'), 'ids': _.map(_.filter(arg, v => v['internal_group'] == 'manuf_account'), v => v['id'])},
                        {'name': _t('Other'), 'ids': _.map(_.filter(arg, v => !['asset', 'liability', 'equity', 'income', 'expense', 'trading_account'].includes(v['internal_group'])), v => v['id'])},
                    ]
                });
            }
